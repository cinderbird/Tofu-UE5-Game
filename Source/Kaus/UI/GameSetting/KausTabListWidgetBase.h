...
#include "KausTabListWidgetBase.generated.h"
...
class UKausTabButtonInterface : public UInterface
{
	GENERATED_BODY()
};

class IKausTabButtonInterface
{
	GENERATED_BODY()

public:
	UFUNCTION(BlueprintNativeEvent, Category = "Tab Button")
	void SetTabLabelInfo(const FKausTabDescriptor& TabDescriptor);
};


UCLASS(Blueprintable, BlueprintType, Abstract, meta = (DisableNativeTick))
class KAUS_API UKausTabListWidgetBase : public UCommonTabListWidgetBase
{
	GENERATED_BODY()
	
public:
	UFUNCTION(BlueprintCallable, BlueprintPure, Category = "Tab List")
	bool GetPreregisteredTabInfo(const FName TabNameId, FKausTabDescriptor& OutTabInfo);

	/** Helper method to get at all the preregistered tab infos */
	const TArray<FKausTabDescriptor>& GetAllPreregisteredTabInfos() { return PreregisteredTabInfoArray; }
	...
	UFUNCTION(BlueprintCallable, Category = "Tab List")
	bool RegisterDynamicTab(const FKausTabDescriptor& TabDescriptor);
	...
	DECLARE_EVENT_TwoParams(UKausTabListWidgetBase, FOnTabContentCreatedNative, FName /* TabId */, UCommonUserWidget* /* TabWidget */);
...
private:
	...
	TArray<FKausTabDescriptor> PreregisteredTabInfoArray;
	 ...
	 */
	UPROPERTY()
	TMap<FName, FKausTabDescriptor> PendingTabLabelInfoMap;
};
