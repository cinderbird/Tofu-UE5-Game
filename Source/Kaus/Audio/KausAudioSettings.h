...
#include "KausAudioSettings.generated.h"
...
USTRUCT()
struct KAUS_API FKausSubmixEffectChainMap
{
	GENERATED_BODY()
 ...
 * 
 */
UCLASS(config = Game, defaultconfig, meta = (DisplayName = "KausAudioSettings"))
class KAUS_API UKausAudioSettings : public UDeveloperSettings
{
	GENERATED_BODY()
...
public:
	...
	TArray<FKausSubmixEffectChainMap> HDRAudioSubmixEffectChain;
	
	/** Submix Processing Chains to achieve low dynamic range audio output */
	UPROPERTY(config, EditAnywhere, Category = EffectSettings)
	TArray<FKausSubmixEffectChainMap> LDRAudioSubmixEffectChain;
