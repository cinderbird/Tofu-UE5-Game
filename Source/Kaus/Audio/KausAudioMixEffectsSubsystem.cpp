...
#include "KausAudioSettings.h"
#include "Setting/KausSettingsLocal.h"
#include "Sound/SoundEffectSubmix.h"
#include "SoundControlBus.h"
#include "SoundControlBusMix.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(KausAudioMixEffectsSubsystem)

class FSubsystemCollectionBase;

void UKausAudioMixEffectsSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	Super::Initialize(Collection);
}

void UKausAudioMixEffectsSubsystem::Deinitialize()
{
...
bool UKausAudioMixEffectsSubsystem::ShouldCreateSubsystem(UObject* Outer) const
{
...
void UKausAudioMixEffectsSubsystem::PostInitialize()
{
	if (const UKausAudioSettings* KausAudioSettings = GetDefault<UKausAudioSettings>())
	{
		if (UObject* ObjPath = KausAudioSettings->DefaultControlBusMix.TryLoad())
		{
			...
			{
				ensureMsgf(SoundControlBusMix, TEXT("Default Control Bus Mix reference missing from Kaus Audio Settings."));
			}
		}

		if (UObject* ObjPath = KausAudioSettings->LoadingScreenControlBusMix.TryLoad())
		{
			if (USoundControlBusMix* SoundControlBusMix = Cast<USoundControlBusMix>(ObjPath))
			{
			...
			{
				ensureMsgf(SoundControlBusMix, TEXT("Loading Screen Control Bus Mix reference missing from Kaus Audio Settings."));
			}
		}
		...
		if (UObject* ObjPath = KausAudioSettings->UserSettingsControlBusMix.TryLoad())
		{
			...
			{
				ensureMsgf(SoundControlBusMix, TEXT("User Control Bus Mix reference missing from Kaus Audio Settings."));
			}
		}
		...
		for (const FKausSubmixEffectChainMap& SoftSubmixEffectChain : KausAudioSettings->HDRAudioSubmixEffectChain)
		{
			FKausAudioSubmixEffectsChain NewEffectChain;
		...
		for (const FKausSubmixEffectChainMap& SoftSubmixEffectChain : KausAudioSettings->LDRAudioSubmixEffectChain)
		{
			FKausAudioSubmixEffectsChain NewEffectChain;
...
void UKausAudioMixEffectsSubsystem::OnWorldBeginPlay(UWorld& InWorld)
{
	if (const UWorld* World = InWorld.GetWorld())
	{
		...
		if (const UKausSettingsLocal* KausSettingsLocal = GetDefault<UKausSettingsLocal>())
		{
			...
			if (UserMix)
			{
				...
				{
					const FSoundControlBusMixStage OverallControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, OverallControlBus, KausSettingsLocal->GetOverallVolume());
					const FSoundControlBusMixStage MusicControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, MusicControlBus, KausSettingsLocal->GetMusicVolume());
					const FSoundControlBusMixStage SoundFXControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, SoundFXControlBus, KausSettingsLocal->GetSoundFXVolume());
					const FSoundControlBusMixStage DialogueControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, DialogueControlBus, KausSettingsLocal->GetDialogueVolume());
			...
			ApplyDynamicRangeEffectsChains(KausSettingsLocal->IsHDRAudioModeEnabled());
		}
	}
}

void UKausAudioMixEffectsSubsystem::ApplyDynamicRangeEffectsChains(bool bHDRAudio)
{
	TArray<FKausAudioSubmixEffectsChain> AudioSubmixEffectsChainToApply;
	TArray<FKausAudioSubmixEffectsChain> AudioSubmixEffectsChainToClear;
	...
	for (const FKausAudioSubmixEffectsChain& EffectChainToClear : AudioSubmixEffectsChainToClear)
	{
		bool bAddToList = true;

		for (const FKausAudioSubmixEffectsChain& SubmixEffectChain : AudioSubmixEffectsChainToApply)
		{
...
void UKausAudioMixEffectsSubsystem::OnLoadingScreenStatusChanged(bool bShowingLoadingScreen)
{
	ApplyOrRemoveLoadingScreenMix(bShowingLoadingScreen);
}

void UKausAudioMixEffectsSubsystem::ApplyOrRemoveLoadingScreenMix(bool bWantsLoadingScreenMix)
{
...
bool UKausAudioMixEffectsSubsystem::DoesSupportWorldType(const EWorldType::Type World) const
{
