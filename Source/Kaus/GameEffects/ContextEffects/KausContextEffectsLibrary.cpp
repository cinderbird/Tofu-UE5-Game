...
void UKausContextEffectsLibrary::GetEffects(const FGameplayTag Effect, const FGameplayTagContainer Context, 
...
void UKausContextEffectsLibrary::LoadEffects()
{
...
EContextEffectsLibraryLoadState UKausContextEffectsLibrary::GetContextEffectsLibraryLoadState()
{
	// Return current Load State
	return EffectsLoadState;
}

void UKausContextEffectsLibrary::LoadEffectsInternal()
{
	// TODO Add Async Loading for Libraries

	// Copy data for async load
	TArray<FKausContextEffects> LocalContextEffects = ContextEffects;

	// Prepare Active Context Effects Array
	TArray<UKausActiveContextEffects*> ActiveContextEffectsArray;

	// Loop through Context Effects
	for (const FKausContextEffects& ContextEffect : LocalContextEffects)
	{
		// Make sure Tags are Valid
		if (ContextEffect.EffectTag.IsValid() && ContextEffect.Context.IsValid())
		{
			// Create new Active Context Effect
			UKausActiveContextEffects* NewActiveContextEffects = NewObject<UKausActiveContextEffects>(this);
	...
	this->KausContextEffectLibraryLoadingComplete(ActiveContextEffectsArray);
}

void UKausContextEffectsLibrary::KausContextEffectLibraryLoadingComplete(
	TArray<UKausActiveContextEffects*> KausActiveContextEffects)
{
	// Flag data as loaded
	...
	ActiveContextEffects.Append(KausActiveContextEffects);
}
