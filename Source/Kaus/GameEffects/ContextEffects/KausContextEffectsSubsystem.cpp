...
#include "GameEffects/ContextEffects/KausContextEffectsSubsystem.h"
...
void UKausContextEffectsSubsystem::SpawnContextEffects(
	...
	, float AudioPitch)
{
	...
	if (TObjectPtr<UKausContextEffectsSet>* EffectsLibrariesSetPtr = ActiveActorEffectsMap.Find(SpawningActor))
	{
		// Validate the pointers from the Map Find
		if (UKausContextEffectsSet* EffectsLibraries = *EffectsLibrariesSetPtr)
		{
			...
			for (UKausContextEffectsLibrary* EffectLibrary : EffectsLibraries->KausContextEffectsLibraries)
			{
...
bool UKausContextEffectsSubsystem::GetContextFromSurfaceType(
	TEnumAsByte<EPhysicalSurface> PhysicalSurface, FGameplayTag& Context)
{
	// Get Project Settings
	if (const UKausContextEffectsSettings* ProjectSettings = GetDefault<UKausContextEffectsSettings>())
	{
...
void UKausContextEffectsSubsystem::LoadAndAddContextEffectsLibraries(AActor* OwningActor,
	TSet<TSoftObjectPtr<UKausContextEffectsLibrary>> ContextEffectsLibraries)
{
	...
	UKausContextEffectsSet* EffectsLibrariesSet = NewObject<UKausContextEffectsSet>(this);
	// Cycle through Libraries getting Soft Obj Refs
	for (const TSoftObjectPtr<UKausContextEffectsLibrary>& ContextEffectSoftObj : ContextEffectsLibraries)
	{
		// Load Library Assets from Soft Obj refs
		// TODO Support Async Loading of Asset Data
		if (UKausContextEffectsLibrary* EffectsLibrary = ContextEffectSoftObj.LoadSynchronous())
		{
			// Call load on valid Libraries
			EffectsLibrary->LoadEffects();

			// Add new library to Set
			EffectsLibrariesSet->KausContextEffectsLibraries.Add(EffectsLibrary);
		}
	}
...
void UKausContextEffectsSubsystem::UnloadAndRemoveContextEffectsLibraries(AActor* OwningActor)
{
