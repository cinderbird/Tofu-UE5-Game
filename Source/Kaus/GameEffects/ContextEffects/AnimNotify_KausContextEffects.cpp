#include "AnimNotify_KausContextEffects.h"
#include "GameEffects/ContextEffects/KausContextEffectsLibrary.h"
#include "KausContextEffectsInterface.h"
...
UAnimNotify_KausContextEffects::UAnimNotify_KausContextEffects()
{
}

void UAnimNotify_KausContextEffects::PostLoad()
{
	Super::PostLoad();
}

#if WITH_EDITOR
void UAnimNotify_KausContextEffects::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);
}
#endif

FString UAnimNotify_KausContextEffects::GetNotifyName_Implementation() const
{
...
void UAnimNotify_KausContextEffects::Notify(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation,
	const FAnimNotifyEventReference& EventReference)
{
	...
	if (MeshComp)
	{
		...
		if (AActor* OwningActor = MeshComp->GetOwner())
		{
			...
			TArray<UObject*> KausContextEffectImplementingObjects;

			// Determine if the Owning Actor is one of the Objects that implements the Context Effects Interface
			if (OwningActor->Implements<UKausContextEffectsInterface>())
			{
				// If so, add it to the Array
				KausContextEffectImplementingObjects.Add(OwningActor);
			}
			...
			for (const auto Component : OwningActor->GetComponents())
			{
				if (Component)
				{
					...
					if (Component->Implements<UKausContextEffectsInterface>())
					{
						KausContextEffectImplementingObjects.Add(Component);
					}
				}
			}

			// Cycle through all objects implementing the Context Effect Interface
			for (UObject* KausContextEffectImplementingObject : KausContextEffectImplementingObjects)
			{
				if (KausContextEffectImplementingObject)
				{
					// If the object is still valid, Execute the AnimMotionEffect Event on it, passing in relevant data
					IKausContextEffectsInterface::Execute_AnimMotionEffect(KausContextEffectImplementingObject,
			...
			if (bPreviewInEditor)
			{
				...
				if (World && World->WorldType == EWorldType::EditorPreview)
				{
					...
					{
						// Check if it is in fact a UKausContextEffectsLibrary type
						if (UKausContextEffectsLibrary* EffectLibrary = Cast<UKausContextEffectsLibrary>(EffectsLibrariesObj))
						{
...
#if WITH_EDITOR
void UAnimNotify_KausContextEffects::ValidateAssociatedAssets()
{
	Super::ValidateAssociatedAssets();
}

void UAnimNotify_KausContextEffects::SetParameters(FGameplayTag EffectIn, FVector LocationOffsetIn, FRotator RotationOffsetIn,
	FKausContextEffectAnimNotifyVFXSettings VFXPropertiesIn, FKausContextEffectAnimNotifyAudioSettings AudioPropertiesIn,
	bool bAttachedIn, FName SocketNameIn, bool bPerformTraceIn, FKausContextEffectAnimNotifyTraceSettings TracePropertiesIn)
{
	Effect = EffectIn;
